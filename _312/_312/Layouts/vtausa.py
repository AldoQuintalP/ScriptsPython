...
def vtausa01(spark, datasource, columnas, client, branch, report):
    ...
    print(f'Imp del reporte: {imp}')
    nombresColumnas = [row[1] for row in imp.collect()]
    print(f'Nombre columnas: {nombresColumnas}')
    ...
    print(f'Data read text: {data}')
    data = data.withColumn("columns", F.split(data["value"], "\\|"))
    print(f'Data with columns: {data}')
    ...
    print(f'Cliente Connect ......... {client}')
    cnn=c.conect(client)
