...
def refoep01(spark, datasource, columnas, client, branch, report):
    ...
    data = data.withColumn("NumeroParte", F.substring( F.col("NumeroParte").cast("string"),1,20))
    ...
    data = data.withColumn("`VentaUnit$`",  F.col("`VentaUnit$`").cast(DecimalType(18,4)))
    ...
    data = data.withColumn("`Utilidad$`", F.col("`Utilidad$`").cast(DecimalType(18,4)))
    data = data.withColumn("Margen", F.col("Margen").cast(DecimalType(15,4)))
